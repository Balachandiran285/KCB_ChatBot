		...
		[
			...
			function ($rootScope, $http, Constants, Value) { // $compile,
				...
				var _renderHTMLPara = function (msgTypeHTML,
					...
					var msgIdAuth = localStorage.getItem("MSGID" + $rootScope.mybotname);
				...
				var _renderhamburger = function (data) {
						...
						for (count = 0; count < optionsData.length; count++) {
							...
							if ((optionsData[count].type).toUpperCase() === ("Hyperlink").toUpperCase()) {
								...
								optionsHtml += '<li><a ng-click="hamburgerMenuClick(\''
									...
									+ '\')" href="javascript:;">'
								if (optionsData[count].text == "Change bot language") {
									optionsHtml += '<img alt="help" src="assets/images/icons/translate.svg">'
								} else {
									optionsHtml += '<img alt="help" src="assets/images/icons/menu.svg">'
								}
					...
					optionsHtml += '<li>'
						...
						+ '<img  src="assets/images/icons/shines.svg" class="right-link-icon" alt="" />'
				...
				var _botShowSlider = function (items, msgid) {
					...
					for (count = 0; count < items.length; count++) {
						...
						if (items[count].imgurl != "") {
							optionsHtml += '<div class="swiper-slide swiper-slide-bg">'
								+ '<img src="'
								+ items[count].imgurl
					...
					caroContainerHtml = '<div class="carousel-container car-custom">'
					...
					var html = _renderHTMLPara(questTxt,
						caroContainerHtml, "bot"
					+ Value.config.botMsgId);
				...
				var _botRenderQuickReply = function (optionsData, userText, msgid) {
					...
					for (count = 0; count < optionsData.length; count++) {
						...
						if (localStorage.getItem("MSGID" + $rootScope.mybotname) == "language") {
							languagearray.push(optionsData[count].title)
							localStorage.setItem("languagearr" + $rootScope.mybotname, languagearray);

						} else {
							localStorage.removeItem("languagearr" + $rootScope.mybotname);
						}


						if (typeof optionsData[count] == "string") {
							ratingflag = true;
							if (optionsData[count].split("~~~")[0] === '&#128077;' || optionsData[count].split("~~~")[0] === '&#128078;') {
								optionsHtml += '<li ng-click="' + "likeDislikeReq('" + optionsData[count] + '\')\">' +
									'<a  href="javascript:;">' + optionsData[count].split('~~~')[0] + '</a>' +
									'</li>';
							}
				...
				'</div>';*/
					...
					var carouselHTML = '<div class="chatting lefty helllo2 chatfade" id="bot' + Value.config.botMsgId + '">' +
						'<div class="chat-container">' +
						'<p>' + userText + '</p>'
					if (localStorage.getItem("MSGID" + $rootScope.mybotname) == "feedback") {

						carouselHTML += '<div class="ratting-container"  quest="' + localStorage.getItem("MSGIDTEXT" + $rootScope.mybotname) + '">'
							+ '<span title="Very Poor"><input type="radio" name="ratting" value="1" ></span>'
							+ '<span title="Poor"><input type="radio" name="ratting" value="2"></span>'
							+ '<span title="Average"><input type="radio" name="ratting" value="3"></span>'
							+ '<span title="Good"><input type="radio" name="ratting" value="4"></span>'
							+ '<span title="Very Good"><input type="radio" name="ratting" value="5"></span>'
						'</div>' +
							'</div>' +
							'<span class="time-right fadein">' + getTimeIn12() + '</span>'
						...
						if (localStorage.getItem("MSGID" + $rootScope.mybotname) == "mediablock") {
							carouselHTML += '</div>' +
								'<span class="time-right fadein">' + getTimeIn12() + '</span>'

								+ '</div><div class="quickreply-container"><ul id="bot' + Value.config.botMsgId + '"  class="lst-quickreply sticky ">' + optionsHtml + '</ul></div>';
						} else {
							carouselHTML += '</div>' +
								'<span class="time-right fadein">' + getTimeIn12() + '</span>'
								+ '</div><div class="quickreply-container"><ul id="bot' + Value.config.botMsgId + '"  class="lst-quickreply non-sticky ">' + optionsHtml + '</ul></div>'
						}
					}
					...
					if (optionsData.length != 0) {
						for (count = 0; count < optionsData.length; count++) {
							...
							if ((optionsData[count].type)
								...
								optionsHtml += '<li><a ng-click="hamburgerMenuClick(\''
									+ optionsData[count].text
									+ '\')" href="javascript:;"><img alt="help" src="images/icons/help-icon.png">'
									+ optionsData[count].text
									+ '</a></li>';
							}
						}
